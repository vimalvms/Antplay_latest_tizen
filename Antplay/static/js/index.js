...
function pairTo(nvhttpHost, onSuccess, onFailure) {
  ...
  nvhttpHost.pollServer(function(ret) {
    ...
    randomNumber = String("0000" + cryptoRand(10000)).slice(-4);
...
function otp_validate() {
  ...
  if (Token != "") {
    var URL = "http://103.182.65.1/" + "api/vmauth";
    $.ajax({
      type: 'POST',
      url: URL,
      dataType: 'json',
...
function addHost() {
  ...
  // $('#continueAddHost').on('click', function() {
    var inputHost = localStorage.getItem("vmip");
    ...
    _nvhttpHost.refreshServerInfoAtAddress(inputHost).then(function(success) {
      ...
      if(saveHosts()) removeClicked(hosts[_nvhttpHost.serverUid]);
